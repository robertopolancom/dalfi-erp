// Pruebas para el estado "No confirmada": una cita "Preaprobada" del chatbot
// que llega a su hora sin confirmarse libera su horario (en vez de bloquear
// la agenda indefinidamente), puede ser reclamada de vuelta si nadie más lo
// toma, o queda "Reprogramada" automáticamente si otra reserva confirmada
// ocupa ese mismo horario primero. Ver también:
// - functions/api/booking/send-reminders.js (quien marca "No confirmada")
...
test("No confirmada: no bloquea la disponibilidad (GET /availability la trata igual que Cancelada)", async () => {
  const doc = {
    ...BASE_DOC,
    reservas: [
      { reservaID: "RES-NC-1", fecha: "2026-08-11", hora: "10:00", horaFin: "11:00", duracionMin: 60, colaboradorID: "COL-1", estado: "No confirmada" },
    ],
  };
  ...
  assert.ok(data.slots.some((s) => s.time === "10:00"), "10:00 debe seguir disponible pese a la cita No confirmada");
});

test("confirm.js: reservar el mismo horario de una cita No confirmada la reprograma automáticamente (bump)", async () => {
  const doc = {
    ...BASE_DOC,
    reservas: [
      {
        reservaID: "RES-NC-2", fecha: "2026-08-11", hora: "10:00", horaFin: "11:00", duracionMin: 60,
        colaboradorID: "COL-1", colaboradorNombre: "Ana Pérez", estado: "No confirmada",
  ...
  assert.equal(bumped.estado, "Reprogramada");
...
test("confirm.js: confirmar una reserva ya Reprogramada (ALREADY_REASSIGNED) se rechaza en vez de resucitarla", async () => {
  const doc = {
    ...BASE_DOC,
    reservas: [
      { reservaID: "RES-REPROG-1", fecha: "2026-08-11", hora: "10:00", colaboradorID: "COL-1", estado: "Reprogramada" },
    ],
  };
  const env = createNestedMockEnv(doc);
  const req = new Request("https://localhost/api/booking/confirm", {
  ...
  assert.equal(env.getDoc().data.reservas[0].estado, "Reprogramada", "el estado no debe haber cambiado");
});

test("confirm.js: confirmar una reserva todavía No confirmada (nadie más tomó el horario) sí funciona", async () => {
  const doc = {
    ...BASE_DOC,
    reservas: [
      { reservaID: "RES-NC-3", fecha: "2026-08-11", hora: "10:00", colaboradorID: "COL-1", estado: "No confirmada" },
    ],
  };
  const env = createNestedMockEnv(doc);
  const req = new Request("https://localhost/api/booking/confirm", {
...
test("send-reminders.js: escala y marca No confirmada una cita Preaprobada cuya hora ya pasó sin confirmarse", async () => {
  // Fecha muy en el pasado a propósito, para no depender de la zona horaria
  // local al parsear "fecha T hora" (checkPreapprovedConfirmationReminder no
  // usa offset) — cualquier ambigüedad de horas cabe sobrada dentro de años.
  const doc = {
    ...BASE_DOC,
    reservas: [
      {
        reservaID: "RES-ESCALATE-1", fecha: "2020-01-01", hora: "08:00", colaboradorID: "COL-1",
        estado: "Preaprobada", canalOrigen: "chatbot_whatsapp", telefono: "8095553333",
        clienteNombre: "Clienta Tarde", created_at: "2019-12-31T20:00:00.000Z",
      },
    ],
  };
  const env = createNestedMockEnv(doc, {
  ...
  await withFakeGlobalFetch(
    ...
    async () => {
      ...
      assert.equal(env.getDoc().data.reservas[0].estado, "No confirmada");
    },
  );
});
