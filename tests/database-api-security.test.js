...
test("configuración exige canManageConfiguration y no se hereda de otros dominios", async () => {
  const { onRequestPut } = await import(apiUrl);
  const current = baseDocument();
  const proposed = structuredClone(current);
  proposed.data.servicios.push({ servicioID: "SER-1", servicio: "Manicura" });

  let fake = fakeSupabase({
    document: current,
    profileRow: profile({ can_manage_billing: true, can_manage_configuration: false }),
  });
  try {
    const denied = await onRequestPut({
      request: request("PUT", { data: proposed, expectedUpdatedAt: "2026-07-25T10:00:00Z" }),
  ...
  fake = fakeSupabase({
    document: current,
    profileRow: profile({ can_manage_invoices: false, can_manage_configuration: true }),
  });
  try {
    const allowed = await onRequestPut({
      request: request("PUT", { data: proposed, expectedUpdatedAt: "2026-07-25T10:00:00Z" }),
