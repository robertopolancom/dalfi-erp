...
test("defaultPermissionsForRole: operador no recibe NINGUN permiso administrativo por defecto", async () => {
  ...
  assert.strictEqual(permissions.can_manage_inventory, false);
  assert.strictEqual(permissions.can_manage_payroll, false);
  assert.strictEqual(permissions.can_manage_accounts, false);
  assert.strictEqual(permissions.can_manage_configuration, false);
...
test("defaultPermissionsForRole: administradora/propietario reciben todos los permisos administrativos", async () => {
  ...
  for (const role of ["administradora", "administrador", "propietaria", "propietario"]) {
    ...
    assert.strictEqual(permissions.can_manage_inventory, true, role);
    assert.strictEqual(permissions.can_manage_payroll, true, role);
    assert.strictEqual(permissions.can_manage_accounts, true, role);
    assert.strictEqual(permissions.can_manage_configuration, true, role);
...
test("defaultPermissionsForRole: contador/contadora revisan cuentas y auditoria pero NUNCA permisos administrativos", async () => {
  ...
  for (const role of ["contador", "contadora"]) {
    ...
    assert.strictEqual(permissions.can_manage_inventory, false, role);
    assert.strictEqual(permissions.can_manage_payroll, false, role);
    assert.strictEqual(permissions.can_manage_accounts, false, role);
    assert.strictEqual(permissions.can_manage_configuration, false, role);
