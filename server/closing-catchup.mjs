// Catch-up de cierres de caja, del lado del SERVIDOR: genera los cierres diarios "sin
// confirmar" que falten sin depender de que nadie tenga el ERP abierto en el navegador.
//
// Por qué existe este archivo (2026-09-10): la misma lógica vivía en
// functions/api/run-closing-catchup.js, una Cloudflare Pages Function que dejó de
// desplegarse cuando se borró dalfi-erp.pages.dev (2026-08-24) -- y el Worker que debía
// llamarla (workers/closing-cron) nunca llegó a desplegarse. Resultado: desde el
// 2026-08-12 no había cron ninguno y los cierres solo se creaban cuando alguien abría el
// ERP, porque ensureProvisionalClosings() en outputs/app.js corre en el navegador. Esto
// es ese port, ya contra Postgres y servido por Render.
//
// Es un port reducido de ensureProvisionalClosings()/normalizeLegacyClosings() de
// outputs/app.js: cubre generar los cierres que faltan y normalizar los antiguos sin
// closingType, nada más. Por cada fecha vencida sin cierre crea EXACTAMENTE dos registros
// -- uno closingType:"register" (caja) y uno closingType:"treasury" (bancos, caja fuerte,
// caja chica) -- nunca uno por cuenta.
//
// Todo lo de aquí es puro: recibe y muta el `data` del documento, no toca red ni base de
// datos. Quien lo llama (la ruta en server/app.mjs) se encarga de leer, guardar y
...
// Corre el catch-up sobre el `data` del documento del ERP, MUTÁNDOLO en sitio.
// `now` se inyecta para poder fijar la hora en las pruebas: la elegibilidad depende de la
// hora de Santo Domingo (un día pasado siempre es elegible; el día en curso solo a partir
// de las 23:59 -- ver isEligible()).
// Devuelve cuántos cierres creó y cuántos antiguos normalizó: con 0 y 0, quien llama no
// necesita guardar nada.
export function runClosingCatchUp(data, now = new Date()) {
  if (!data || typeof data !== "object") throw new Error("El documento del ERP no trae data.");
  const today = localDateStringInZone(now);
  const { hour, minute } = nowPartsInZone(now);
