...
-- El único cliente de esta base es el servicio de Render, así que no hay roles
-- públicos a los que revocar permisos. `crear_reserva` sigue siendo el único
-- camino para insertar una reserva: es donde vive el lock que evita la
-- sobreventa.
...
-- El identificador del administrador llega como parámetro: quien autentica es
-- el servidor Express, que solo llama a esta función con una sesión válida.
...
begin
  if p_admin_id is null
     or not exists (select 1 from administradores a where a.id = p_admin_id and a.activo)
  then
  ...
  if not p_aprobar then
    update solicitudes_cambio
       set estado = 'rechazada', nota_resolucion = p_nota,
           resuelta_en = now(), resuelta_por = p_admin_id
