...
--
-- Ejecutar sobre una base limpia con las migraciones aplicadas:
--   psql -f supabase/tests/_stub_supabase.sql   (solo en Postgres local)
--   psql -f supabase/migrations/000*.sql
--   psql -v ON_ERROR_STOP=1 -f supabase/tests/reglas.test.sql
--
...
begin
  insert into auth.users (email) values ('contadora@test.do') returning id into admin;
  ...
  r := resolver_solicitud(sol, true, 'Aprobado');
  ...
  select id into sol from solicitudes_cambio where estado = 'pendiente' limit 1;
  r := resolver_solicitud(sol, true, null);
